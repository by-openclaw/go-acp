...
func runProducer(ctx context.Context, protoName string, args []string) error {
	...
	var (
		...
		proxyUpstream  = fs.String("proxy-upstream", "", "snell-rollcall only: host[:port] of a real frame to front instead of a tree — our own IPShare, e.g. 10.6.255.113:2050. Every routed request a client sends is carried to that frame on a connection of the client's own and its answers carried back; the proxy unit and its routing nodes are still answered here. The frame is probed once at start to learn its unit and identity. Needs --proxy-subnet; --tree / --manifest become optional.")
