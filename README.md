...
| EVS Cerebrum NB | XML over WebSocket (ws/wss) | 40007 (configurable) | 🟡 PR #144 — read path **fully live-verified 2026-04-27** against two real Cerebrums (11 + 27 devices). 12 single-OBTAIN-per-pcap CLI verbs (connect / list-devices / list-routers / list-categories / list-salvo-groups / walk / device-details / device-value / category-details / list-salvo-instances / salvo-instance-details / listen). 6 live-wire decoder shapes pinned (LIST `IP=`/INSTANCE, DETAILS sub-tree, VALUE, CATEGORY_DETAILS items grid, INSTANCE_LIST/DETAILS, multi-INSTANCE). Routing model locked: `IP_ADDRESS=0.0.0.0` + `DEVICE_TYPE=ROUTER` sentinel for the central route-master (per spec §4.1 + reference driver). Route ACTION verb pending — needs sandbox sign-off (TEST-YOB src/dst pair). | deferred | [internal/cerebrum-nb/CLAUDE.md](internal/cerebrum-nb/CLAUDE.md) · [internal/cerebrum-nb/docs/consumer.md](internal/cerebrum-nb/docs/consumer.md) · [internal/cerebrum-nb/docs/keys.md](internal/cerebrum-nb/docs/keys.md) |
