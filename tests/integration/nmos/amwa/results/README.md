# AMWA NMOS Testing Tool â€” Node conformance evidence
...
8 suites Ã— every applicable IS-04 minor. 31 runs, one JSON + one node
log each.

## Current score â€” 76.8% coverage, 99.0% of executed passed, 0 Fail
...
| Count | Why | Real gap? |
|---:|---|---|
| 252 | `ENABLE_AUTH` is False | **yes** â€” BCP-003-02 Authorization is unimplemented |
| 15 | `DNS_SD_MODE` is not 'unicast' | **yes** â€” unicast DNS-SD is NOT covered by these runs |
| 4 | "Replaced by 'auto' test" or similar | no â€” superseded |
| 36 | Could Not Test â€” fixture too thin | **yes, ours** â€” see below |
| 20 | Manual | needs a human |

The 36 Could Not Test are all fixture gaps, not code gaps:

- IS-08-01/-02 `auto_channelmapping_14/15` â€” the fixture schedules no
  channel-mapping activations for the tool to observe.
- IS-08-01 `test_13/14/15` â€” no input forbids a route, prevents
  re-ordering, or constrains by block; the constraint-enforcement code
  paths exist but nothing in the bundle exercises them.
- IS-07-02 `test_06` â€” no matching resources in the bundle.

### The 11 Warnings

- IS-04-01 `test_12` Ã—3 (v1.0â€“v1.2): "no matching mDNS announcement" â€”
  an artifact of the docker-bridge run mode, where the tool cannot see
  the node's multicast. Not present in LAN-mode runs.
- IS-05-01 `test_27/28` Ã—8: activation entries set "later than
  expected" â€” scheduled-activation timing tolerance, worth a look but
...
indistinguishable from an RFC 6762 Â§10.1 goodbye â€” and the watcher
...
â€” TTL==0 keeps one meaning on every backend). Pinned by
...
- **This run's fixture carries the min/full archetypes** â€” including a
...
- **dnssd record TTL** (above) â€” cleared all 12 IS-09-02 fails.
- **IS-07 events seed from the full bundle**, not the minor projection â€”
...
Update 2026-08-30: with internet egress restored on the tooling host,
`is-04-03` and `is-05-01` were rescored (auth mode, ONLINE image with
avahi): is-04-03 is now 24/0 - the P2P announce round passes. The two
`is-05-01 test_27/28` activation-timing warnings persist on the online
image too, i.e. tool/scheduler timing variance (the suite's own text
says so), not an image artefact. Net set: 683 Pass, 0 Fail, 2 benign
