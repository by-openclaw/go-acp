...
| All Ember+ traffic           | `s101_emberplus`              |
| Ember+ keep-alive only       | `s101.command == 0x01 or s101.command == 0x02` |
| Ember+ Glow elements         | `glow`                        |

### Decode As… (non-default ports)

If your provider runs on a port we don't register by default, right-click a
packet → **Decode As…** → pick the appropriate protocol (`s101_emberplus`,
...
| Ember+ frames show only raw bytes       | `s101_emberplus` heuristic disabled under **Analyze → Enabled Protocols**. |
