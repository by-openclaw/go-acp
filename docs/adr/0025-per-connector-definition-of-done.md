...
| 4 | **DM + manifest fixture — committed, or generated** | A **committed** fixture under `internal/<proto>/testdata/integration-test/{manifest,dm}/` (the emberplus pattern — the durable artefact, captured once from a real device/oracle walk, trimmed and version-pinned per [`internal/manifest/TEMPLATE.md`](../../internal/manifest/TEMPLATE.md)) **or** a Go generator under `internal/<proto>/integration/` that writes the same fixture. Either form is acceptable; the bar is that the producer builds its frame **from the repo alone** with no host dependency. | Tests **MUST NOT** `t.Skip` for a missing fixture: a committed fixture is always present; a generator must write it before use. The committed fixture is the preferred form — it is byte-stable, reviewable, and replayable in CI without a device. |
...
| Probel SW-P-08 | All §3.2 commands + general AND extended form per command. Form selection is a tested boundary decision (`needsExtended()` ceilings per internal/probel-sw08p/CLAUDE.md). Salvo behaviour per internal/probel-sw08p/CLAUDE.md. |
| Probel SW-P-02 | All §3 commands; protect-blocks-connect with state echo (internal/probel-sw02p/CLAUDE.md). |
...
- 2026-06-14 — deliverable 4 reconciled: a **committed** fixture under
  `internal/<proto>/testdata/integration-test/{manifest,dm}/` (the emberplus
  pattern) now explicitly satisfies #4 as the preferred form, with a Go
  generator as an accepted alternative. Both meet the original intent (producer
