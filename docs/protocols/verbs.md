...
| snell-rollcall | common | `serve --tree` or `--manifest --cache-dir`; `--generation 16\|32` picks the wire generation, `--generation-16-slots` makes named cards speak the older one; `--unit N` sets the unit address every card is reached at; `--proxy-subnet NNNN` presents as a RollCall IP Proxy with the served tree (`--proxy-frame UU`) or a real frame (`--proxy-upstream host[:port]`, no tree needed) behind that route |
