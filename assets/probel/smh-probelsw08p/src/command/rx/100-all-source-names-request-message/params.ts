/**
 * AllSourceNamesRequestMessage command parameters
 *
 * @export
 * @interface AllSourceNamesRequestMessageCommandParams
 */
export interface AllSourceNamesRequestMessageCommandParams {
    /**
     * Matrix ID of the matrix to monitor
     * + Range [0 - 255]
     *
     * @type {number}
     * @memberof AllSourceNamesRequestMessageCommandParams
     */
    matrixId: number;

    /**
     * Level ID of the matrix to monitor
     * + Range [0 - 255]
     *
     * @type {number}
     * @memberof AllSourceNamesRequestMessageCommandParams
     */
    levelId: number;
}

/**
 * Utility class providing extra functionalities on the AllSourceNamesRequestMessage command parameters
 *
 * @export
 * @class CommandParamsUtility
 */
export class CommandParamsUtility {
    /**
     * Gets a textual representation of the command parameters
     *
     * @static
     * @param {AllSourceNamesRequestMessageCommandParams} params the command parameters
     * @returns {string} the command parameters textual representation
     * @memberof CommandParamsUtility
     */
    static toString(params: AllSourceNamesRequestMessageCommandParams): string {
        return `Matrix:${params.matrixId}, Level:${params.levelId}`;
    }
}
