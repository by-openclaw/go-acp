...
func TestIdentity(t *testing.T) {
	v := New()
	if v.SpecID() != "bcp-008-02" || v.APIVer() != "v1.0" || v.SpecPatch() != "v1.0.0" {
...
// Only a descriptor that claims the NcSenderMonitor name is checked;
...
func TestValidate(t *testing.T) {
	cases := []struct {
		...
		{"canonical", `{"classId":[1,2,2,2],"name":"NcSenderMonitor"}`, "", 0},
		{"other class is out of scope", `{"classId":[1,2,2,9],"name":"NcSomethingElse"}`, "", 0},
		{"wrong last element", `{"classId":[1,2,2,9],"name":"NcSenderMonitor"}`, "bcp_008_02_class_id_mismatch", spec.SeverityError},
		{"truncated lineage", `{"classId":[1,2,2],"name":"NcSenderMonitor"}`, "bcp_008_02_class_id_mismatch", spec.SeverityError},
		{"classId missing", `{"name":"NcSenderMonitor"}`, "bcp_008_02_class_id_mismatch", spec.SeverityError},
		{"not json", `{`, "bcp_008_02_decode_error", spec.SeverityError},
	}
