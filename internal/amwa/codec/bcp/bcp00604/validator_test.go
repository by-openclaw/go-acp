...
const (
	rightFormat = MuxFormatURN
	wrongFormat = "urn:x-nmos:format:video"
)

func TestIdentity(t *testing.T) {
	v := New()
	if v.SpecID() != "bcp-006-04" || v.APIVer() != "v1.0" || v.SpecPatch() != "v1.0.0" {
...
func TestValidate(t *testing.T) {
	cases := []struct {
		...
		{"canonical pair", `{"format":"` + rightFormat + `","media_type":"` + MPEGTSMediaType + `"}`, "", 0},
		{"other media type is out of scope", `{"format":"` + wrongFormat + `","media_type":"video/raw"}`, "", 0},
		{"no fields at all", `{}`, "", 0},
		{"wrong format", `{"format":"` + wrongFormat + `","media_type":"` + MPEGTSMediaType + `"}`, "bcp_006_04_format_mismatch", spec.SeverityWarn},
		{"missing format", `{"media_type":"` + MPEGTSMediaType + `"}`, "bcp_006_04_format_mismatch", spec.SeverityWarn},
		{"not json", `{`, "bcp_006_04_decode_error", spec.SeverityError},
	}
...
func TestMismatchDetailNamesBothSides(t *testing.T) {
	events := New().Validate([]byte(`{"format":"` + wrongFormat + `","media_type":"` + MPEGTSMediaType + `"}`))
	if len(events) != 1 || !strings.Contains(events[0].Detail, MPEGTSMediaType) || !strings.Contains(events[0].Detail, wrongFormat) {
