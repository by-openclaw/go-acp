// Package v10 is the AMWA NMOS IS-04 v1.0.3 wire codec.
//
// SELF-CONTAINED BY DESIGN. Everything v1.0-specific lives in this
...
// deliberate — a change to v1.0 must be incapable of altering how any
// other version behaves.
//
// It holds NO validation rules. Every rule comes from AMWA's own
// v1.0.3 JSON Schemas, shipped verbatim in is04/schemas/ and applied by
...
//
//	Encode  marshal -> drop what v1.0 lacks -> schema check is FATAL
...
// the latest patch within v1.0, and the schema set in is04/schemas/v1.0.3.
const SpecPatch = "v1.0.3"

// drop names the properties a v1.0 payload MUST NOT carry, because
// IS-04 did not define them until a later minor.
//
// Paths are dot-separated; a segment ending in "[]" applies to every
// element of that array. This table is v1.0's alone — the other
...
// ValidateNode reports whether n can be served on the v1.0 wire.
...
// ValidateDevice reports whether d can be served on the v1.0 wire.
...
// ValidateSource reports whether s can be served on the v1.0 wire.
...
// ValidateFlow reports whether f can be served on the v1.0 wire.
...
// ValidateSender reports whether s can be served on the v1.0 wire.
...
// ValidateReceiver reports whether r can be served on the v1.0 wire.
func (Codec) ValidateReceiver(r is04.Receiver) error {
	_, err := encode("receiver", r)
	return err
}

// encode is the one egress path: marshal, drop what v1.0 does not
// define, then check the result against AMWA's own v1.0.3 schema.
//
...
func encode(kind string, x any) ([]byte, error) {
	...
	switch kind {
	case "node", "device":
		// v1.0 permits description/tags either way, but the AMWA
		// IS-04-02 downgrade tests expect a v1.0 body not to carry
		// them when empty. A shape rule, not a version delta.
		if raw, err = dropEmpty(raw, "description", "tags"); err != nil {
			return nil, err
		}
	}
...
// marshal is json.Marshal behind a package variable. The strip and
// dropEmpty stages in encode only fail on bytes that are not JSON, and
// json.Marshal never produces those, so their error branches cannot be
// reached by any input. A test swaps this to hand encode such bytes
// and prove the failures are reported, not swallowed. Production never
// reassigns it.
var marshal = json.Marshal

// reportDeviations checks a peer's payload against AMWA's v1.0.3 schema
...
//
// Private to v10 on purpose: the other minors carry their own copy.
...
// dropEmpty removes the named top-level keys when their value is
// empty — "", null, {} or [].
func dropEmpty(raw []byte, keys ...string) ([]byte, error) {
