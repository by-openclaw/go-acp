...
type plant struct {
	...
	// pages lets one path answer with a Link: rel="next" chain.
...
func (p *plant) ServeHTTP(w http.ResponseWriter, r *http.Request) {
	...
	if chunks, ok := p.pages[r.URL.Path]; ok {
		...
		if idx < len(chunks)-1 || p.stuck {
			...
			w.Header().Set("Link", fmt.Sprintf(`<%s?paging.since=%d>; rel="next"`, r.URL.Path, next))
		}
...
func TestManifestHrefFollowedVerbatim(t *testing.T) {
	...
	sdp, err := os.ReadFile(filepath.Join(got.Dir, "sdp", "sender_44444444-4444-4444-8444-444444444444.sdp"))
	if err != nil {
...
func TestEmbeddedReceiverSDP(t *testing.T) {
	...
	if _, err := os.Stat(filepath.Join(got.Dir, "sdp", "receivers_"+id+"_active.sdp")); err != nil {
...
func TestVisitedTargetsCaptureOnce(t *testing.T) {
	visited := map[string]bool{"h:1": true}
