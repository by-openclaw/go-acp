...
func TestHandleUpdateNameSource(t *testing.T) {
	...
	frame := iprobel.EncodeUpdateNameRequest(iprobel.UpdateNameRequestParams{
		NameType: iprobel.UpdateNameSource, NameLength: iprobel.NameLen4,
...
func TestHandleUpdateNameDestAssoc(t *testing.T) {
	srv := newComplianceServer(t)
	frame := iprobel.EncodeUpdateNameRequest(iprobel.UpdateNameRequestParams{
		NameType: iprobel.UpdateNameDestAssoc, NameLength: iprobel.NameLen4,
...
func TestHandleUpdateNameUMDIgnored(t *testing.T) {
	srv := newComplianceServer(t)
	frame := iprobel.EncodeUpdateNameRequest(iprobel.UpdateNameRequestParams{
		NameType: iprobel.UpdateNameUMDLabel, NameLength: iprobel.NameLen4,
		MatrixID: 0, LevelID: 0, FirstID: 0,
		Names: []string{"UMD1"},
	})
	if _, err := srv.handle(frame); err != nil {
