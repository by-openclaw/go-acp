...
func (s *server) dispatch(f codec.Frame) (handlerResult, error) {
	...
	case codec.RxSourceLockStatusRequest:
		return s.handleSourceLockStatusRequest(f)
	...
	case codec.RxExtendedProtectTallyDumpRequest:
		return s.handleExtendedProtectTallyDumpRequest(f)
