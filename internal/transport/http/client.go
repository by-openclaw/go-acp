...
func (c *Client) applyAuth(ctx context.Context, req *stdhttp.Request) error {
	...
	if err != nil {
		return fmt.Errorf("http: obtain access token: %w", err)
	}
...
func (c *Client) GetJSON(ctx context.Context, url string, dst any) error {
	if dst == nil {
		return fmt.Errorf("http: GetJSON: dst must not be nil")
	}
	...
	if resp.StatusCode != stdhttp.StatusOK {
		...
		return fmt.Errorf("http: GET %s: HTTP %d: %s",
			url, resp.StatusCode, strings.TrimSpace(string(body)))
	}

	ct := resp.Header.Get("Content-Type")
	if !isJSONContentType(ct) {
		return fmt.Errorf("http: GET %s: unexpected Content-Type %q (want application/json)", url, ct)
	}
	...
	if int64(len(body)) > max {
		return fmt.Errorf("http: response body exceeds %d bytes", max)
	}
	...
	if d.More() {
		return fmt.Errorf("http: trailing JSON content in body")
	}
	return nil
}

// GetBytes issues a GET and returns the raw body, capped at MaxBody.
//
// For payloads a caller decodes itself, or that are not JSON at all — the
// CCM connector pulls a Neuron's own OpenAPI document, which is YAML. The
// cap is the reason this exists rather than an io.ReadAll at the call site:
// a device answering with a gigabyte should produce an error, not an OOM.
//
// A non-200 is an error carrying the status; the body is still read (capped)
// so the peer's explanation reaches the operator.
func (c *Client) GetBytes(ctx context.Context, url string) ([]byte, error) {
	...
	if int64(len(body)) > max {
		return nil, fmt.Errorf("http: response body exceeds %d bytes", max)
	}
	if resp.StatusCode != stdhttp.StatusOK {
		return nil, fmt.Errorf("http: GET %s: HTTP %d: %s",
			url, resp.StatusCode, strings.TrimSpace(string(body)))
	}
	return body, nil
}
...
func (c *Client) GetJSONPage(ctx context.Context, url string, dst any) (string, error) {
	if dst == nil {
		return "", fmt.Errorf("http: GetJSONPage: dst must not be nil")
	}
	req, err := stdhttp.NewRequestWithContext(ctx, stdhttp.MethodGet, url, nil)
	if err != nil {
		return "", fmt.Errorf("http: build request: %w", err)
...
func (c *Client) GetJSONPageLinks(ctx context.Context, url string, dst any) (next, prev string, err error) {
	...
	if err := json.Unmarshal(body, dst); err != nil {
		return "", "", fmt.Errorf("http: decode %s: %w", url, err)
	}
