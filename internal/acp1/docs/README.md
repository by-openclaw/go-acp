# ACP1 — Axon Synapse Protocol v1.4

| Role | Doc | Status |
|---|---|---|
| **Feature & ADR status** | [status.md](status.md) | numbered matrix: every feature, state, evidence, ADR |
| **Verbs & config reference** | [verbs.md](verbs.md) | every verb + transport/redundancy/export/import/tree/ensure/wireshark/ansible, with real samples |
| Operator runbook | [runbook.md](runbook.md) | ✓ shipping |
| Consumer | [consumer.md](consumer.md) | ✓ shipping |
| Provider | [provider.md](provider.md) | ✓ shipping — serves canonical tree as ACP1 device over UDP (Mode A) |

## Spec documents

| Document | Path | Description |
|---|---|---|
| AXON ACP v1.4 | [AXON-ACP_v1_4.pdf](../../../internal/acp1/assets/AXON-ACP_v1_4.pdf) | Full specification |
| Wireshark dissector | [dhs_acpv1.lua](../../../internal/acp1/wireshark/dhs_acpv1.lua) | Byte-exact reference |
