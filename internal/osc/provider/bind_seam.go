...
// cannot reach. Transparent nil/identity-default pattern (cf. the sibling
// internal/tsl/provider/bind_seam.go): in production every hook is nil, so
// bind behaves EXACTLY as originally written — the short-circuit setsockopt
// sequence, the c.Control dispatch-error guard, and the *net.UDPConn type
// assertion are all preserved verbatim. Tests install a hook to drive an
// otherwise-dead defensive arm, never to weaken it.
//
