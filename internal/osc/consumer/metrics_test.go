...
// one exposed none, so `--metrics-addr` served a scrape with no osc
// series in it at all.
func TestPluginExposesMetrics(t *testing.T) {
	p := (&Factory{version: V10}).New(plugin.Deps{Logger: slog.New(slog.NewTextHandler(io.Discard, nil))}).(*Plugin)
	if p.Metrics() == nil {
