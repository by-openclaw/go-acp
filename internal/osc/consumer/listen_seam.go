...
// pattern as the listen seams in the sibling tsl consumer
// (internal/tsl/consumer/listen_seam.go) and the decode seams elsewhere
// in the repo: in production every seam is the identity behaviour, so the
// guarded branches behave exactly as written; a test swaps the seam to
// drive the otherwise-dead defensive arm, never to weaken it.
//
// Why each seam exists:
//
//   - rawControl: net.ListenConfig.Control receives a syscall.RawConn
//     whose Control method only errors when the underlying fd is already
//     invalid. For a socket the runtime is in the middle of binding that
//     never happens, so the `if err := c.Control(...); err != nil` arm in
//     listen is unreachable from a real bind. rawControl defaults to
//     calling c.Control directly; a test substitutes a function returning
//     an error to drive that arm.
//
