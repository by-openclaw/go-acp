...
func findACP2Error(t *testing.T, captureName string) (*codec.ACP2Message, codec.ACP2ErrStatus) {
	t.Helper()
	...
	for _, r := range recs {
		...
		if msg.Type == codec.ACP2TypeError {
			return msg, codec.ACP2ErrStatus(msg.Func)
		}
	}
...
func TestCompliance_ErrNoAccess(t *testing.T) {
	...
	if stat != codec.ErrNoAccess {
		t.Fatalf("expected ErrNoAccess (%d), got %d", codec.ErrNoAccess, stat)
	}
	...
	err := msg.ToACP2Error()
	ace, ok := err.(*codec.ACP2Error)
	if !ok {
		t.Fatalf("ToACP2Error type = %T, want *codec.ACP2Error", err)
	}
	if ace.Status != codec.ErrNoAccess {
		t.Errorf("ACP2Error.Status = %d, want %d", ace.Status, codec.ErrNoAccess)
	}
}
...
func TestCompliance_ErrInvalidObj(t *testing.T) {
	...
	if stat != codec.ErrInvalidObjID {
		t.Fatalf("expected ErrInvalidObjID (%d), got %d", codec.ErrInvalidObjID, stat)
	}
	...
	if ace.Status != codec.ErrInvalidObjID {
		t.Errorf("ACP2Error.Status = %d, want %d", ace.Status, codec.ErrInvalidObjID)
	}
}
...
func TestEventForErrStatus_All(t *testing.T) {
	cases := []struct {
		stat codec.ACP2ErrStatus
		want string
	}{
		{codec.ErrProtocol, acp2.ProtocolErrorReceived},
		{codec.ErrInvalidObjID, acp2.InvalidObjectReceived},
		{codec.ErrInvalidIdx, acp2.InvalidIndexReceived},
		{codec.ErrInvalidPID, acp2.InvalidPidReceived},
		{codec.ErrNoAccess, acp2.AccessDeniedReceived},
		{codec.ErrInvalidValue, acp2.InvalidValueReceived},
	}
	for _, c := range cases {
	...
	if got := acp2.EventForErrStatus(codec.ACP2ErrStatus(99)); got != "" {
